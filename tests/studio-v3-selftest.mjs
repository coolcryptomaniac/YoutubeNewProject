...
assert.equal(TEMPLATE_COUNT,VIDEO_TEMPLATES.length);assert.ok(TEMPLATE_COUNT>=72);assert.equal(themeById('naru-shadow').id,'naru-shadow');assert.ok(VIDEO_TEMPLATES.some(x=>x.theme==='phonk-noir'));assert.ok(VIDEO_TEMPLATES.some(x=>x.theme==='naru-rain'));assert.ok(VIDEO_TEMPLATES.some(x=>x.theme==='naru-sage'));
const cues=buildLyricCues('[Verse]\nपहली रात\nदूसरी याद\n[Chorus]\nफिर वही प्यार',12);assert.equal(cues.length,3);assert.ok(cues.every(x=>x.section==='Song'));assert.ok(cues.every(x=>x.end>x.start));
...
assert.match(wrangler,/\[ai\]\s*\nbinding = "AI"/);assert.match(wrangler,/RIDGE_PAID_FALLBACK = "false"/);assert.match(wrangler,/DIRECTOR_TEXT_MODEL = "@cf\/zai-org\/glm-4\.7-flash"/);assert.ok(!/NVIDIA_TEXT_MODEL/.test(wrangler),'retired NVIDIA model must not be a deploy default');assert.ok(!/wrangler secret put/.test(deploy),'ordinary code deploys must preserve existing Worker secrets instead of rewriting/deploying each one');assert.match(deploy,/Deploy without rewriting secrets/);assert.match(deploy,/RIDGE_R2_READY/);assert.match(deploy,/RELEASE_MEDIA/);assert.match(deploy,/push:\n\s+branches: \[main\]/);assert.match(deploy,/cloud\/\*\*/);assert.match(deploy,/Post-deploy live smoke/);assert.match(deploy,/Vusic automation smoke/);assert.match(deploy,/ridge-cloud-media\.founder-f53\.workers\.dev/);assert.match(deploy,/node \.\.\/tests\/ridge-cloud-live-smoke\.mjs/);
