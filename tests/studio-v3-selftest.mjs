...
assert.equal(TEMPLATE_COUNT,VIDEO_TEMPLATES.length);assert.ok(TEMPLATE_COUNT>=72);assert.equal(themeById('naru-neon').id,'naru-neon');assert.ok(VIDEO_TEMPLATES.some(x=>x.theme==='phonk-noir'));assert.ok(VIDEO_TEMPLATES.some(x=>x.theme==='naru-rain'));assert.ok(VIDEO_TEMPLATES.some(x=>x.theme==='naru-sage'));
