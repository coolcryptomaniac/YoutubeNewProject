...
function writeStable(next){
  const clean={
    version:1,
    groq:String(next.groq||''),
    pollinations:String(next.pollinations||''),
    youtubeClient:String(next.youtubeClient||''),
    pexelsLegacy:String(next.pexelsLegacy||''),
    updatedAt:new Date().toISOString()
  };
  localStorage.setItem(STABLE_KEY,JSON.stringify(clean));
  return clean;
}
...
export function migrateCredentials(){
  ...
  const migrated={
    groq:old.groq||first(sessionStorage,['ridge.v25.groq','ridge.v2.simple.groq','ridge.v2.plus.groq'])||first(localStorage,['ridge.v25.groqKey','ridge.v2.groqKey','ridge.groqKey']),
    pollinations:old.pollinations||first(localStorage,['ridge.v25.pollinationsKey','ridge.v2.pollinationsKey','ridge.pollinationsKey']),
    youtubeClient:old.youtubeClient||first(localStorage,['ridge.v25.youtubeClient','ridge.v2.googleClientId','ridge.v2.youtubeClient','ridge.googleClientId']),
...
export function saveCredentials(partial={}){
  return writeStable({...readStable(),...partial});
}
...
export function applyCredentials(c=readStable()){
  ...
  for(const [k,sel] of Object.entries(map)){
    const el=$(sel);if(!el||!c[k])continue;
    if(!el.value)el.value=c[k];
  }
  return c;
}
...
export async function encryptedBackup(passphrase){
  ...
  const payload=JSON.stringify(readStable());
  const salt=crypto.getRandomValues(new Uint8Array(16)),iv=crypto.getRandomValues(new Uint8Array(12));
  const key=await derive(passphrase,salt);
  const data=await crypto.subtle.encrypt({name:'AES-GCM',iv},key,new TextEncoder().encode(payload));
...
export async function restoreEncrypted(text,passphrase){
  ...
  const key=await derive(passphrase,b64ToBytes(box.salt));
  ...
  return writeStable(restored);
}

function flash(msg,kind='ok'){
  const e=$('#credentialState');if(!e)return;e.textContent=msg;e.dataset.kind=kind;
}
function downloadText(text,name){
  const a=document.createElement('a'),u=URL.createObjectURL(new Blob([text],{type:'application/json'}));a.href=u;a.download=name;a.click();setTimeout(()=>URL.revokeObjectURL(u),2000);
}

async function init(){
  const c=migrateCredentials();applyCredentials(c);
  flash(`Saved on this device${c.groq?' · Groq':''}${c.pollinations?' · Pollinations':''}${c.youtubeClient?' · YouTube':''}.`);
  ...
  $('#backupCredentials')?.addEventListener('click',async()=>{
    try{const pass=prompt('Choose a passphrase for the encrypted settings backup:');if(!pass)return;downloadText(await encryptedBackup(pass),`ridge-v2-settings-${new Date().toISOString().slice(0,10)}.json`);flash('Encrypted settings backup downloaded.')}catch(e){flash(e.message,'err')}
  });
  $('#restoreCredentials')?.addEventListener('click',()=>$('#credentialFile')?.click());
  $('#credentialFile')?.addEventListener('change',async e=>{
    const file=e.target.files?.[0];if(!file)return;
    try{const pass=prompt('Enter the backup passphrase:');if(!pass)return;const restored=await restoreEncrypted(await file.text(),pass);applyCredentials(restored);flash('Settings restored and saved on this device.');}
    catch(err){flash('Restore failed: '+err.message,'err')}finally{e.target.value=''}
  });
  $('#forgetCredentials')?.addEventListener('click',()=>{
    if(!confirm('Forget saved provider credentials on this browser?'))return;
    localStorage.removeItem(STABLE_KEY);for(const id of['groqKey','pollinationsKey','youtubeClient'])if($(id))$(id).value='';flash('Saved credentials removed.','warn');
  });
}
