...
export class GroqSongBrain{
  ...
  async lockMeaning({transcript='',workingLyrics='',filename='',idea='',language='Hindi'}={}){
    ...
    return this.chat([
      {role:'system',content:'You are a meticulous song editor and music-video story director. First understand the song; do not write marketing copy yet. Output one JSON object only.'},
      {role:'user',content:`Create a LOCKED semantic record for this song. This record is the only source allowed for title, description and music-video scenes later.\n\nSelected language: ${language}\n${guide}\nFilename: ${filename}\nCreator idea: ${idea||'none'}\nWorking lyrics/transcript:\n${source}\n\nRules:\n- Do not translate the song into another language.\n- Correct obvious transcription mistakes only when strongly supported by context.\n- Identify one exact central story, one chorus/hook meaning, 4-8 concrete visual objects/places/actions, and the emotional change from beginning to end.\n- Create 8-16 chronological story_beats that follow the actual lyrical progression. Each beat must explain what the relevant lyric means and turn it into one coherent filmable visual idea.\n- Repeated choruses may revisit a visual motif, but evolve framing/action so the video feels intentional rather than repetitive.\n- Prefer human actions, locations, objects, weather and visual metaphors that can be searched or generated. Do not make every line literal.\n- energy is a number from 0.1 to 1.0 following emotional/musical intensity.\n- camera must be one of wide, medium, close, detail. motion should be still, drift, push, pull, track or handheld. transition should be cut, dissolve, fade, push, whip or flash.\n- Reject generic AI wording, invented plot points, clickbait and unrelated anime/franchise references.\n- If the transcript is uncertain, say so instead of inventing facts.\n\nReturn {"language":"${language}","story":"one precise sentence","hook_meaning":"one precise sentence","clean_lyrics":"cleaned lyrics in the same language/script","mood":["3-6 words"],"visual_anchors":["4-8 concrete nouns/actions"],"emotional_arc":"beginning -> change -> ending","story_beats":[{"section":"Verse/Chorus/etc","lyric_hint":"short exact or paraphrased lyric cue","meaning":"what this part means","visual":"one coherent filmable scene idea","query":"short stock/generation search phrase","energy":0.5,"camera":"wide","motion":"drift","transition":"dissolve"}],"uncertainty":"short note or empty string","title_anchor":"2-5 words taken from or tightly implied by the hook"}.`}
    ],{temperature:.22,maxTokens:3900});
  }
...
export function fallbackPackage({filename='Original Song',language='English',lyrics='',idea=''}={}){
  ...
  return {title,description:[title,idea||'Original music video.','\nMusic/visual credits: add your credits here.'].filter(Boolean).join('\n'),hashtags:['#music','#musicvideo'],tags:['music','music video'],thumbnail_headline:title.split(/\s+/).slice(0,4).join(' '),visual_queries:[idea||title],intro:title,outro:'Thanks for listening',edit_energy:'medium',suggested_theme:'clean',clean_lyrics:lyrics,language,story:idea||title,hook_meaning:idea||title,visual_anchors:[],story_beats:[],emotional_arc:''};
}