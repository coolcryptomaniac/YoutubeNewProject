...
export const PUBLIC_DEFAULTS={youtubeClient:'1061806252746-tg3g74uovrt9e72osbloiq0poe9lllgh.apps.googleusercontent.com'};
let dbPromise=null;
...
export class MediaLibrary{
  ...
  async scan({maxFiles=500,maxDepth=5}={}){if(!this.root)throw new Error('Choose a media folder first.');const state=await permission(this.root,true);if(state!=='granted')throw new Error('Folder permission is required to scan media.');const out=[];const walk=async(dir,parts=[],depth=0)=>{if(depth>maxDepth||out.length>=maxFiles)return;for await(const [name,handle] of dir.entries()){if(out.length>=maxFiles)break;if(handle.kind==='directory'){if(!name.startsWith('.'))await walk(handle,[...parts,name],depth+1);continue}if(!safeExt(name))continue;try{const f=await handle.getFile(),kind=mediaType(f);if(!kind)continue;out.push({id:'disk:'+([...parts,name].join('/')),path:[...parts,name],name:f.name,size:f.size,type:f.type,kind,lastModified:f.lastModified,source:'folder'})}catch{}}};await walk(this.root,[],0);const transient=this.items.filter(x=>x.source!=='folder');this.items=[...out,...transient];this.lastScan=Date.now();try{await put('media-index',out);await put('media-last-scan',this.lastScan)}catch{}return out}
