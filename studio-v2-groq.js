...
export class GroqHelper{
  ...
  async analyzeSong({transcript='',workingLyrics='',workingTitle='',theme='',language='English',channel=''}={}){
    ...
    const prompt=`Analyze this ORIGINAL song and prepare everything needed for a polished YouTube music video. Return JSON only.\nWorking title: ${workingTitle||'Untitled'}\nTheme hint: ${theme||'infer it from the song'}\nLanguage hint: ${language}\nChannel/artist: ${channel||'independent creator'}\nTranscribed/working lyrics:\n${source}\n\nReturn {"title":"best truthful music-video title under 70 characters","clean_lyrics":"cleaned lyrics reconstructed from the transcript; keep original language; add simple [Verse]/[Chorus]/[Bridge] labels where reasonably inferable","description":"natural YouTube description with strongest two lines first, short song meaning/context, credits placeholders and 3-5 hashtags","hashtags":["3-5 hashtags"],"tags":["12-18 useful search tags"],"thumbnail_headline":"2-5 words","thumbnail_prompt":"cinematic 16:9 thumbnail background prompt, one strong focal subject, dramatic light, negative space for headline, no text/logo/watermark","pexels_queries":["4 concrete landscape stock-video search phrases related to this exact song"],"scene_prompts":["6 cinematic image prompts related to exact lyric/story moments, visually coherent but varied"],"intro":"short elegant intro card under 8 words","outro":"short end-card CTA under 10 words","mood":"short mood","visual_theme":"short visual direction"}. Avoid keyword stuffing, fake claims and copyrighted franchise/character names.`;
    const out=cleanJson(await this.chat([{role:'system',content:'You are a music-video editor who turns an actual song into accurate metadata and visual direction. Output valid JSON only.'},{role:'user',content:prompt}],{temperature:.64,maxTokens:3600}));
