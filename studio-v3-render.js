...
async function cleanupRidgeStorage(){
  try{if('caches' in self){for(const name of await caches.keys())if(/^ridge-(?:render|temp|preview)-/i.test(name))await caches.delete(name)}}catch{}
  try{
    ...
    dated.sort((a,b)=>b.t-a.t);for(const x of dated.slice(1))try{await root.removeEntry(x.name)}catch{}
  }catch{}
}
...
export class RenderEngine{
  ...
  quality(aspect='landscape'){
    ...
    if(aspect==='vertical'){if(emergency)return {w:240,h:426,fps:15,bitrate:650_000,tier:'ultra-safe'};return {w:540,h:960,fps:24,bitrate:2_100_000,tier:'safe'}}
    if(emergency)return {w:426,h:240,fps:15,bitrate:650_000,tier:'ultra-safe'};return {w:960,h:540,fps:24,bitrate:2_400_000,tier:'safe'}
  }
  resetSequence(project){const procedural=this.proceduralExport||isMobile();this.sequence=procedural?[]:this.library.chooseSequence(12,project.title||project.idea||'ridge');this.sceneIndex=0;this.storyIndex=-1;this.storyScene=null;this.lastMediaId='';this.lastCut=-99;this.beat.reset();this.lease.clear();if(procedural)return;const first=project.scenePlan?.scenes?.[0],item=first?this.library.chooseForScene(first,{seed:project.title||'ridge'}):this.sequence[0];if(item){this.lastMediaId=item.id;this.lease.request(item).catch(()=>{})}}
  ...
  async render(audioFile,project,{onProgress=()=>{}}={}){
    if(this.rendering)throw new Error('A render is already running.');if(!audioFile)throw new Error('Choose a song first.');this.rendering=true;this.abortFlag=false;this.proceduralExport=isMobile();this.stopPreview(true);this.lease.clear();await cleanupRidgeStorage();try{await this.gestureCtx?.suspend()}catch{}
    const q=this.quality(project.aspect||'landscape');this.canvas.width=q.w;this.canvas.height=q.h;this.lease.maxW=q.w;this.lease.maxH=q.h;this.resetSequence(project);let ac=null,audio=null,url='',ownsUrl=false,stream=null,raf=0,writer=null,chunks=[],chunkBytes=0,pendingBytes=0,visibilityHandler=null,rec=null,flushTimer=0;
    try{
      ...
      const duration=Number.isFinite(audio.duration)?audio.duration:0;if(!duration)throw new Error('Could not read song duration.');if(duration>180&&isMobile()){q.fps=12;q.bitrate=520_000}if(!project.lyricCues?.length)project.lyricCues=buildLyricCues(project.lyrics||'',duration);
      ac=AC();await ac.resume();const src=ac.createMediaElementSource(audio),an=ac.createAnalyser(),dest=ac.createMediaStreamDestination();an.fftSize=128;an.smoothingTimeConstant=.82;src.connect(an);an.connect(dest);const canvasStream=this.canvas.captureStream(q.fps);const vt=canvasStream.getVideoTracks()[0];if(vt)try{vt.contentHint='detail'}catch{}stream=new MediaStream([...canvasStream.getVideoTracks(),...dest.stream.getAudioTracks()]);
      try{rec=new MediaRecorder(stream,mime?{mimeType:mime,videoBitsPerSecond:q.bitrate,audioBitsPerSecond:64000}:undefined)}catch{rec=new MediaRecorder(stream,{videoBitsPerSecond:Math.min(q.bitrate,600_000),audioBitsPerSecond:64000})}
      const freq=new Uint8Array(an.frequencyBinCount);let write=Promise.resolve(),writeError=null,ended=false,lastDraw=0,lastPressureCheck=0;const mobile=isMobile(),pendingLimit=(mobile?4:24)*MB,memoryLimit=mobile?48*MB:280*1024*1024;
      ...
      an.getByteFrequencyData(freq);this.drawFrame(0,duration,bands(freq),project);rec.start(2000);flushTimer=setInterval(()=>{try{if(rec?.state==='recording')rec.requestData()}catch{}},1600);try{await audio.play()}catch{throw new Error('Browser blocked render playback. Tap CREATE VIDEO once more; Ridge will reuse the granted media gesture.')}
      const interval=1000/q.fps;const loop=ts=>{if(ended||this.abortFlag)return;if(ts-lastPressureCheck>1000){lastPressureCheck=ts;const m=memorySnapshot();if((m.limit&&m.ratio>.68)||pendingBytes>pendingLimit*.72){this.lease.clear();if(m.limit&&m.ratio>.78){this.abortFlag=true;this.onState({type:'warn',message:'Ridge stopped before browser memory reached Android kill pressure.'});return}}}if(ts-lastDraw>=interval){lastDraw=ts;an.getByteFrequencyData(freq);this.drawFrame(audio.currentTime,duration,bands(freq),project);onProgress(unit(audio.currentTime/duration))}raf=requestAnimationFrame(loop)};raf=requestAnimationFrame(loop);
      while(!ended&&!this.abortFlag)await sleep(180);cancelAnimationFrame(raf);clearInterval(flushTimer);flushTimer=0;if(!audio.paused)audio.pause();if(rec.state!=='inactive')rec.stop();await stopped;await write;if(writeError)throw writeError;if(this.abortFlag){this.forceEmergency=true;throw new Error('Render stopped safely before a browser crash. Ridge is now locked to the ultra-safe tier; press CREATE once more.')}const file=writer?await finishOpfs(writer):new File(chunks,name,{type:mime||'video/webm'});onProgress(1);return {file,mime:file.type||mime||'video/webm',quality:q,duration,sceneCount:project.scenePlan?.scenes?.length||0}
    }catch(e){if(writer)try{await writer.w.abort()}catch{}throw e}
    finally{if(flushTimer)clearInterval(flushTimer);if(visibilityHandler)document.removeEventListener('visibilitychange',visibilityHandler);cancelAnimationFrame(raf);try{if(rec&&rec.state!=='inactive')rec.stop()}catch{}stream?.getTracks().forEach(t=>t.stop());try{audio?.pause()}catch{}try{await ac?.close()}catch{}if(ownsUrl&&url)try{URL.revokeObjectURL(url)}catch{}this._releaseRenderAudio();this.lease.clear();chunks.length=0;this.proceduralExport=false;this.rendering=false}
  }
