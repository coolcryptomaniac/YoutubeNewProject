...
function underMemoryPressure(){const m=memorySnapshot();return (m.limit&&m.ratio>=.72)||m.deviceGB<=2||m.cores<=2}
...
class SceneLease{
  ...
  async _decode(item){
    if(!item)return null;
    const maxPixels=this.maxW*this.maxH*2.5;
    if(item.kind==='video'&&item.source==='remote'){
      ...
      await new Promise((res,rej)=>{let done=false;const finish=fn=>{if(done)return;done=true;v.onloadedmetadata=null;v.onerror=null;fn()};v.onloadedmetadata=()=>finish(res);v.onerror=()=>finish(()=>rej(new Error('A cloud stock clip could not be streamed.')));try{v.load()}catch{}setTimeout(()=>finish(res),4000)});
      if(v.videoWidth*v.videoHeight>maxPixels&&underMemoryPressure()){try{v.pause();v.removeAttribute('src');v.load()}catch{}throw new Error('Skipped an oversized cloud clip to protect browser memory.')}
    ...
    if(item.kind==='image'){
      if(file.size>40*MB&&underMemoryPressure())throw new Error('Skipped an oversized image to protect browser memory.');
      let bitmap;
      try{bitmap=await createImageBitmap(file,{imageOrientation:'from-image',resizeWidth:this.maxW,resizeHeight:this.maxH,resizeQuality:'medium'})}
      catch(e){throw new Error(`Safe resized image decode failed; Ridge skipped the image instead of decoding it full-size. ${e?.message||''}`.trim())}
    ...
    try{
      await new Promise((res,rej)=>{let done=false;const finish=fn=>{if(done)return;done=true;v.onloadedmetadata=null;v.onerror=null;fn()};v.onloadedmetadata=()=>finish(res);v.onerror=()=>finish(()=>rej(new Error('A selected video could not be decoded.')));try{v.load()}catch{}setTimeout(()=>finish(res),3000)});
      if(v.videoWidth*v.videoHeight>maxPixels&&underMemoryPressure())throw new Error('Skipped an oversized local video to protect browser memory.');
      try{await v.play()}catch{}
      return {id:item.id,kind:'video',source:v,release(){try{v.pause();v.removeAttribute('src');v.load();URL.revokeObjectURL(url)}catch{}}};
    }catch(e){try{v.pause();v.removeAttribute('src');v.load();URL.revokeObjectURL(url)}catch{}throw e}
  }
...
export class RenderEngine{
  constructor(canvas,library,{onState=()=>{}}={}){this.canvas=canvas;this.ctx=canvas.getContext('2d',{alpha:false,desynchronized:true});this.library=library;this.onState=onState;this.sequence=[];this.sceneIndex=0;this.storyIndex=-1;this.storyScene=null;this.lastMediaId='';this.lastCut=-99;this.beat=new BeatGate();this.lease=new SceneLease(library,{onState});this.preview=null;this.previewRaf=0;this.previewLast=0;this.rendering=false;this.abortFlag=false;this.gestureCtx=null;this.renderAudio=null;this.renderAudioUrl=''}
  quality(aspect='landscape'){
    const m=memorySnapshot(),mobile=isMobile(),weak=m.deviceGB<=2||m.cores<=2,pressure=m.limit&&m.ratio>=.62;
    if(aspect==='vertical'){if(weak||pressure)return {w:480,h:854,fps:24,bitrate:1_500_000,tier:'emergency'};return mobile||m.deviceGB<=4?{w:540,h:960,fps:24,bitrate:2_100_000,tier:'mobile'}:{w:720,h:1280,fps:30,bitrate:3_600_000,tier:'desktop'}}
    if(weak||pressure)return {w:854,h:480,fps:24,bitrate:1_500_000,tier:'emergency'};return mobile||m.deviceGB<=4?{w:960,h:540,fps:24,bitrate:2_100_000,tier:'mobile'}:{w:1280,h:720,fps:30,bitrate:4_200_000,tier:'desktop'}
  }
  ...
  async render(audioFile,project,{onProgress=()=>{}}={}){
    if(this.rendering)throw new Error('A render is already running.');if(!audioFile)throw new Error('Choose a song first.');this.rendering=true;this.abortFlag=false;this.stopPreview(true);try{await this.gestureCtx?.suspend()}catch{}
    ...
    try{
      ...
      if(!Number.isFinite(audio.duration)||!audio.duration)await new Promise((res,rej)=>{let done=false;const finish=fn=>{if(done)return;done=true;audio.onloadedmetadata=null;audio.onerror=null;fn()};audio.onloadedmetadata=()=>finish(res);audio.onerror=()=>finish(()=>rej(new Error('The song could not be decoded by this browser.')));setTimeout(()=>finish(res),5000)});
      ...
      ac=AC();await ac.resume();const src=ac.createMediaElementSource(audio),an=ac.createAnalyser(),dest=ac.createMediaStreamDestination();an.fftSize=512;an.smoothingTimeConstant=.76;src.connect(an);an.connect(dest);const canvasStream=this.canvas.captureStream(q.fps);stream=new MediaStream([...canvasStream.getVideoTracks(),...dest.stream.getAudioTracks()]);
      try{rec=new MediaRecorder(stream,mime?{mimeType:mime,videoBitsPerSecond:q.bitrate,audioBitsPerSecond:128000}:undefined)}catch{rec=new MediaRecorder(stream,{videoBitsPerSecond:Math.min(q.bitrate,2_000_000),audioBitsPerSecond:96000})}
      const freq=new Uint8Array(an.frequencyBinCount);let write=Promise.resolve(),writeError=null,ended=false,lastDraw=0,lastPressureCheck=0;const stopped=new Promise((res,rej)=>{rec.onerror=e=>rej(e.error||new Error('MediaRecorder failed.'));rec.onstop=res;rec.ondataavailable=e=>{if(!e.data?.size)return;if(writer){pendingBytes+=e.data.size;if(pendingBytes>32*1024*1024){this.abortFlag=true;writeError=new Error('Storage writes are too slow on this device; Ridge stopped before memory could run away.')}const blob=e.data;write=write.then(async()=>{await writer.w.write(blob);pendingBytes=Math.max(0,pendingBytes-blob.size)}).catch(err=>writeError=err)}else{chunks.push(e.data);chunkBytes+=e.data.size;if(chunkBytes>280*1024*1024){this.abortFlag=true;writeError=new Error('This browser cannot stream the export to disk and the in-memory safety limit was reached.')}}}});
      ...
      an.getByteFrequencyData(freq);this.drawFrame(0,duration,bands(freq),project);rec.start(1000);try{await audio.play()}catch{throw new Error('Browser blocked render playback. Tap CREATE VIDEO once more; Ridge will reuse the granted media gesture.')}
      const interval=1000/q.fps;const loop=ts=>{if(ended||this.abortFlag)return;if(ts-lastPressureCheck>2000){lastPressureCheck=ts;const m=memorySnapshot();if(m.limit&&m.ratio>.86){this.abortFlag=true;this.onState({type:'warn',message:'Render stopped before the browser heap reached a crash-level memory state.'});return}}if(ts-lastDraw>=interval){lastDraw=ts;an.getByteFrequencyData(freq);this.drawFrame(audio.currentTime,duration,bands(freq),project);onProgress(unit(audio.currentTime/duration))}raf=requestAnimationFrame(loop)};raf=requestAnimationFrame(loop);
      while(!ended&&!this.abortFlag)await sleep(120);cancelAnimationFrame(raf);if(!audio.paused)audio.pause();if(rec.state!=='inactive')rec.stop();await stopped;await write;if(writeError)throw writeError;if(this.abortFlag)throw new Error('Render stopped safely. Try again; Ridge will automatically use the emergency memory tier if pressure remains high.');const file=writer?await finishOpfs(writer):new File(chunks,name,{type:mime||'video/webm'});onProgress(1);return {file,mime:file.type||mime||'video/webm',quality:q,duration,sceneCount:project.scenePlan?.scenes?.length||0}
...
export {bands,memorySnapshot,underMemoryPressure};
