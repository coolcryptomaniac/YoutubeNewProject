...
export class RenderEngine{
  ...
  resetSequence(project){this.sequence=this.proceduralExport?[]:this.library.chooseSequence(12,project.title||project.idea||'ridge');this.sceneIndex=0;this.storyIndex=-1;this.storyScene=null;this.lastMediaId='';this.lastCut=-99;this.beat.reset();this.lease.clear();if(this.proceduralExport)return;const first=project.scenePlan?.scenes?.[0],item=first?this.library.chooseForScene(first,{seed:project.title||'ridge'}):this.sequence[0];if(item){this.lastMediaId=item.id;this.lease.request(item).catch(()=>{})}}
