...
const DB='ridge-media-bank-v1',STORE='assets';
let dbp=null;
...
async function trim(maxItems=120){
  const all=await req((await tx()).getAll());if(all.length<=maxItems)return;
  all.sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt));const s=await tx('readwrite');for(const x of all.slice(0,all.length-maxItems))s.delete(x.id)
}
...
export async function matchingBank(tags=[],limit=12){const terms=cleanTags(tags),all=await req((await tx()).getAll());return all.map(x=>({x,score:terms.reduce((s,t)=>s+((x.tags||[]).some(v=>v.includes(t)||t.includes(v))?1:0),0)})).sort((a,b)=>b.score-a.score||new Date(b.x.createdAt)-new Date(a.x.createdAt)).slice(0,limit).map(({x})=>x)}
