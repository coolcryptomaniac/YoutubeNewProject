...
export async function distributeVusic(env,input={}){
  ...
  try{
    ...
    await stableGoto(page,urls.release);
    ...
    if(e instanceof VusicProviderError)throw e;
    const detail={steps,url:pageRef?.url?.()||''};
    try{detail.pageText=(await bodyText(pageRef)).slice(0,1200)}catch{}
    throw new VusicProviderError(safe(e?.message||e,500),{status:503,code:'VUSIC_BROWSER_FAILED',detail});
