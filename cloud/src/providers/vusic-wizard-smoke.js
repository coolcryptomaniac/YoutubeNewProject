...
const defaultMediaSelectors={
  audio:'input[type="file"][accept*="audio"],input[type="file"][accept*=".wav"],input[type="file"][accept*=".mp3"],input[name*="audio" i],input[name*="song" i],input[name*="track" i]',
  image:'input[type="file"][accept*="image"],input[type="file"][accept*=".png"],input[type="file"][accept*=".jpg"],input[name*="art" i],input[name*="cover" i]'
};
...
async function launchBrowser(env){let last;for(let attempt=0;attempt<4;attempt++){try{return await puppeteer.launch(env.BROWSER)}catch(e){last=e;if(!/429|rate limit/i.test(String(e?.message||e)))throw e;await sleep(4000*(attempt+1))}}throw last}
...
async function attachRemoteFile(page,selector,url,name,mime,kind){
  let last=null;
...
export async function vusicWizardSmoke(env,input={}){
  ...
  const audioSelector=safe(env.VUSIC_AUDIO_SELECTOR||defaultMediaSelectors.audio,700),imageSelector=safe(env.VUSIC_ARTWORK_SELECTOR||defaultMediaSelectors.image,700);
  let browser;
  try{
    browser=await launchBrowser(env);const page=await browser.newPage();
    ...
    const art=await attachRemoteFile(page,imageSelector,artworkUrl,safe(input.artworkName||'ridge-canary.png',180),safe(input.artworkType||'image/png',120),'image');await sleep(500);
    const audio=await attachRemoteFile(page,audioSelector,audioUrl,safe(input.audioName||'ridge-canary.wav',180),safe(input.audioType||'audio/wav',120),'audio');
