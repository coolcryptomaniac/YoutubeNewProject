...
async function youtubeRoute(request,env){const u=new URL(request.url);
  if(u.pathname==='/api/youtube/capabilities'&&request.method==='GET')return json({ok:true,...youtubeCapabilities(env)});
  if(u.pathname==='/api/youtube/oauth/callback'&&request.method==='GET'){try{const r=await youtubeOauthCallback(request,env),to=safe(r.returnTo||'/pipeline.html',500);return new Response(`<!doctype html><meta charset="utf-8"><title>YouTube connected</title><script>try{if(window.opener){window.opener.postMessage({type:'ridge-youtube-connected'},'*');window.close()}else{location.replace(${JSON.stringify(to)})}}catch(e){location.replace(${JSON.stringify(to)})}</script><p>YouTube connected to Ridge. You can close this window.</p>`,{headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}})}catch(e){return new Response(`<!doctype html><meta charset="utf-8"><title>YouTube connection failed</title><p>${safe(e?.message||e,500).replace(/[<>&]/g,'')}</p>`,{status:400,headers:{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'}})}}
