...
async function stageUpload(request,env){
  ...
  const id=crypto.randomUUID(),ext=(file.name.match(/\.[a-z0-9]{1,8}$/i)||[''])[0].toLowerCase(),key=`release-stage/${id}${ext}`,expires=Date.now()+6*60*60*1000;
  ...
  const origin=new URL(request.url).origin;return json({ok:true,id,key,url:`${origin}/api/release/stage/${encodeURIComponent(id)}${ext}`,name:file.name,type:file.type,size:file.size,expiresAt:new Date(expires).toISOString()});
}
async function stageGet(request,env){
  if(!env.RELEASE_MEDIA)return json({error:'staging unavailable'},404);const u=new URL(request.url),name=u.pathname.split('/').pop()||'';if(!/^[0-9a-f-]{36}(?:\.[a-z0-9]{1,8})?$/i.test(name))return json({error:'bad stage id'},400);
  const obj=await env.RELEASE_MEDIA.get(`release-stage/${name}`);if(!obj)return json({error:'not found'},404);const expires=Number(obj.customMetadata?.expires||0);if(expires&&Date.now()>expires){await env.RELEASE_MEDIA.delete(`release-stage/${name}`);return json({error:'expired'},410);}const h=new Headers(cors);obj.writeHttpMetadata(h);h.set('Cache-Control','private, max-age=300');h.set('Content-Disposition',`inline; filename="${safe(obj.customMetadata?.name||name,160).replace(/"/g,'')}"`);return new Response(obj.body,{headers:h});
}
async function stageDelete(request,env){if(!env.RELEASE_MEDIA)return json({error:'staging unavailable'},404);const body=await request.json().catch(()=>({})),key=safe(body.key,500);if(!key.startsWith('release-stage/'))return json({error:'valid stage key required'},400);await env.RELEASE_MEDIA.delete(key);return json({ok:true});}
