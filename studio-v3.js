...
function log(text,kind=''){const box=$('#log');if(!box)return;const d=document.createElement('div');d.className=kind;d.textContent=`${new Date().toLocaleTimeString()} · ${text}`;box.prepend(d);while(box.children.length>30)box.lastElementChild.remove()}
...
async function createVideo(){
  ...
  try{
    await engine.prepareUserGesture(state.song);projectFromUI();await analyzeSong();setStatus('Preparing local media sequence…');engine.resetSequence(state.project);await refreshPreview();
...
function bind(){
  $('#songFile').onchange=e=>loadSong(e.target.files?.[0]);$('#folderFiles').onchange=e=>{state.library.importFiles(e.target.files);closeFirstRun();updateMediaState();refreshPreview()};
  $('#chooseFolder').onclick=chooseFolder;$('#firstRunFolder').onclick=chooseFolder;$('#firstRunSkip').onclick=()=>{closeFirstRun();setStatus('No folder linked. Ready-made procedural themes still work.','ok')};
  $('#reconnectFolder').onclick=reconnectFolder;$('#rescanFolder').onclick=async()=>{try{await state.library.scan();await updateMediaState();setStatus('Folder scan refreshed.','ok')}catch(e){setStatus(e.message,'warn')}};$('#forgetFolder').onclick=async()=>{await state.library.forgetFolder();await updateMediaState();refreshPreview()};
